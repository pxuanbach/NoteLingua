...
const createHighlightValidation = [
  ...
  body('file_hash').notEmpty().withMessage('File hash is required'),
  body('content.text')
    .notEmpty()
    .withMessage('Highlight text is required')
    .isLength({ min: 1, max: 1000 })
    .withMessage('Highlight text must be between 1 and 1000 characters'),
  body('position.boundingRect.x1').isNumeric().withMessage('Bounding rect x1 must be a number'),
  body('position.boundingRect.y1').isNumeric().withMessage('Bounding rect y1 must be a number'),
  body('position.boundingRect.x2').isNumeric().withMessage('Bounding rect x2 must be a number'),
  body('position.boundingRect.y2').isNumeric().withMessage('Bounding rect y2 must be a number'),
  ...
  body('position.rects').isArray().withMessage('Rects must be an array'),
  body('position.rects.*.x1').isNumeric().withMessage('Rect x1 must be a number'),
  body('position.rects.*.y1').isNumeric().withMessage('Rect y1 must be a number'),
  body('position.rects.*.x2').isNumeric().withMessage('Rect x2 must be a number'),
  body('position.rects.*.y2').isNumeric().withMessage('Rect y2 must be a number'),
  body('position.rects.*.width').isNumeric().withMessage('Rect width must be a number'),
  body('position.rects.*.height').isNumeric().withMessage('Rect height must be a number'),
  body('comment.text')
    .optional()
    .isLength({ max: 1000 })
...
router.get(
  ...
  authenticateToken,
  [
    param('documentId').isMongoId().withMessage('Document ID must be a valid MongoDB ObjectId'),
...
router.get(
  ...
  authenticateToken,
  [
    param('fileHash').notEmpty().withMessage('File hash is required'),
    query('page').optional().isInt({ min: 1 }).withMessage('Page must be a positive integer'),
    query('limit')
      .optional()
      .isInt({ min: 1, max: 100 })
...
router.get(
  ...
  async (req, res) => {
    try {
      const result = await highlightsService.searchHighlights(req.user.id, req.query.q, {
        page: req.query.page,
...
router.get(
  '/:highlightId',
  authenticateToken,
  [param('highlightId').isMongoId().withMessage('Highlight ID must be a valid MongoDB ObjectId')],
  handleValidationErrors,
  async (req, res) => {
    try {
...
router.delete(
  ...
  async (req, res) => {
    try {
      await highlightsService.deleteHighlight(req.params.highlightId, req.user.id);
